...
class ExpRunner():
    ...
    def run_experiments(self, exp_name, n_epochs, batch_size=2):
        ...
        train_data = Brains(crop_size=(64,64), proper_crop_proba=0.3)
    ...
    def train_epoch(self, n_epoch, dataloader, i):
        ...
        #         print(preds.shape, masks_batch.shape)
            ...
            self.writer.add_scalar(self.exp_name+'/Cross entropy loss per iter', loss.item(), i)
            dice = dice_score(preds.float(), masks_batch.cuda().float()).mean()
            self.writer.add_scalar(self.exp_name+'/Dice score', dice.item(), i)
        #         print(X_batch.shape)
        #         print(torchvision.utils.make_grid(X_batch).shape)
            ...
            if torch.any(bool_mask.reshape((1,-1)).byte()==1):
                self.writer.add_image(self.exp_name+'/Input image vs mask vs pred', torch.cat((X_batch[bool_mask.byte()][0], masks_batch[bool_mask.byte()][0].float(), preds[bool_mask.byte()][0].cpu().float()), dim=1), i)
        ...
        self.writer.add_scalar(self.exp_name+'/Train cross entropy loss per epoch', total_ce_loss, n_epoch)
        self.writer.add_scalar(self.exp_name+'/Train dice per epoch', np.mean(epoch_dice), n_epoch)
    ...
    def validate(self, n_epoch, dataloader):
        ...
        self.writer.add_scalar(self.exp_name+'/Val dice per epoch', total_dice, n_epoch)
        return total_dice
